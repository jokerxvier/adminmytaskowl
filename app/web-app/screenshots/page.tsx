...
export default function ScreenshotPage() {
  ...
  const handleGetScreenshots = async (date: any) => {
    ...
    try {
      ...
      const transformedScreenshots = await Promise.all(
        data.response.map(async (screenshot: any) => {
          ...
          try {
            ...
            // Handle base64 type
              ...
              screenshot.storage_type === "base64" ||
              (screenshot.storage_type === "missing" && screenshot.screenshot)
            ) {
    ...
    <div>
      ...
      <div className="flex flex-col w-full h-full">
        ...
        <div className="flex flex-row gap-4 items-center mb-4">
          ...
          />
          <Button disabled={loading} onPress={handleSearch}>
                  ...
                  {organizations.length > 0 ? (
                    <div>
                      ...
                      <span className="ml-2 text-sm font-normal text-gray-500 dark:text-gray-400">
                        ...
                        {organizations.length === 1 ? "result" : "results"})
                      </span>
                    </div>
                  ) : (
                    <div>
                  ...
                  <Card className="p-4 h-full flex flex-col">
                    <div className="flex flex-row mb-4 items-center justify-between ">
                      <div className="flex flex-col gap-2">
                        <p className="font-lg truncate font-bold">
                          {user.name}
                        </p>
                      ...
                      <div className="flex flex-col gap-2">
                        ...
                        <p className="text-sm text-gray-500">
                          ...
                          <strong className="text-default-500">
                            {user.is_blurred === 1 ? "On" : "Off"}
                          </strong>
                        </p>
                        <p className="text-sm text-gray-500">
                          Interval:{" "}
                          <strong className="text-default-500">
                            {user.ss_interval} Minutes
                          </strong>
                        </p>
                      </div>
                    </div>
                    <Button onPress={() => handleOpenModal(user)}>
                      View Screenshots
                    </Button>
                  </Card>
                </div>
            ...
            <div>
              {loading && (
                <div className="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm flex items-center justify-center z-50">
                  <Spinner
                    label={`Loading Screenshots of ${selectedUser.name} on ${date}`}
                    variant="simple"
                  />
                </div>
              )}
              <ModalHeader className="flex flex-col gap-1 text-center">
                {selectedUser.name}'s Screenshots
              </ModalHeader>
                        ...
                        <div className="flex flex-col gap-2 p-3 h-full">
                          ...
                          <div className="flex flex-row items-center justify-between gap-2">
                            ...
                            <Code className="text-sm font-medium truncate">
                              Group ID: {screenshot.screenshotGroupID}
                            </Code>
                            <p className="text-sm font-medium truncate">
                          ...
                          <div className="relative aspect-video overflow-hidden rounded-md bg-gray-100">
                            <Image
                              ...
                              onClick={() =>
                                screenshot.isAvailable &&
                                handleOpenFullscreen(screenshot.displaySource)
                              }
                            />
                          </div>
      ...
      >
        <ModalContent>
          ...
          <ModalFooter className="absolute top-4 right-4">
            <Button isIconOnly color="danger" onPress={onFullscreenClose}>
              <FaExpand />
            </Button>
          </ModalFooter>
        </ModalContent>
